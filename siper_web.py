...
async def main():
                ...
                for tname in target_agents:
                    ...
                    if tname == agent.config.agent_name and agent.config.avatar:
                        ...
                        if candidate.is_file():
                            ...
                            break
                    # Try agent dir avatar.png
                    if not avatar_path:
                        candidate = Path(os.path.dirname(__file__)) / "agents" / tname / "avatar.png"
                        if candidate.is_file():
                            avatar_path = candidate
                            break
    ...
    def api_get_sessions():
        sessions = []
        agents_dir = Path(os.path.dirname(__file__)) / "agents"
        # Collect from all agent session databases
        agent_dirs = [agents_dir / "default"]
        if agents_dir.exists():
            for d in agents_dir.iterdir():
                if d.is_dir() and d.name != "default" and (d / "sessions.db").exists():
                    agent_dirs.append(d)
        for agent_dir in agent_dirs:
            ...
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    # Check message count
                    cursor2 = conn.cursor()
                    cursor2.execute("SELECT COUNT(*) as cnt FROM messages WHERE session_id = ?", (sid,))
                    msg_row = cursor2.fetchone()
                    msg_count = msg_row["cnt"] if msg_row else 0
                    if msg_count == 0:
                        continue
                    # Get last message
                    cursor2.execute("SELECT content, timestamp FROM messages WHERE session_id = ? ORDER BY timestamp DESC LIMIT 1", (sid,))
    ...
    def api_control_gateway(body):
        ...
        if action == "restart" and service_name:
            ...
            if service_name in ("HTTP Server", "WebSocket"):
                ...
                try:
                    # Use existing agent LLM config for restart
                    cur = agent.llm_client
                    if cur:
                        api_key = cur.api_key
                        ...
                        model = ""
                    if not api_key:
                        return {"success": False, "error": "No API Key available. Set LONGCAT_API_KEY or configure model in Web UI."}
                    agent.configure_llm(
                        api_key=api_key,
                        base_url=base_url,
                        model=model,
