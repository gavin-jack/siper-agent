...
class ModelConfig:
    ...
    name: str = ""           # model name, e.g. "LongCat-2.0-Preview"
...
class AgentConfig:
    ...
    llm_timeout: int = 120          # LLM API call timeout in seconds
...
class AIAgent:
    ...
    def configure_llm(
        ...
        # Vision client for image understanding (uses sensenova or other vision-capable API)
    ...
    async def process_message(
        ...
        try:
            ...
            if (self._skill_pre_filter_enabled
                ...
                try:
                    pre_filtered = self.skill_pre_filter.pre_filter(
                        user_input=message,
    ...
    def _smart_truncate_history(
        ...
        Strategy: keep newest messages, always keep at least 1 exchange (2 msgs),
                  drop oldest tool_result round-trips first.
        """
        ...
        # Try keeping N most recent messages, binary search for best fit
        ...
        if best < len(history):
            result = [{'role': 'system', 'content': '[... 早期对话已截断 ...]'}] + result
    ...
    async def _build_user_content(self, message: str) -> Any:
        ...
        # Fallback: if no vision client, embed images as base64 data URLs in OpenAI multimodal format
        if img_matches:
            clean_message = img_pattern.sub('', message).strip()
            content_parts = []
            if clean_message:
                content_parts.append({"type": "text", "text": clean_message})
            for img_path in img_matches:
                img_path = img_path.strip()
                p = Path(img_path)
                if not p.exists() or not p.is_file():
                    content_parts.append({"type": "text", "text": f"[图片文件: {img_path} - 文件不存在]"})
                    continue
                try:
                    raw = p.read_bytes()
                    ...
                    content_parts.append({"type": "text", "text": f"[图片文件: {p.name} - 读取失败: {e}]"})
    ...
    async def _handle_tool_calls(
        ...
        # If LLM returned tool_calls without text content, stream_callback was never
        # invoked so the frontend has no bubble to attach progress to.  Trigger
        # stream_start now by calling stream_callback with empty content (which
        # sends stream_start but no visible text).
        ...
        if tool_results and self.llm_client is not None:
            try:
                ...
                extended = list(messages or self.conversation_history)
...
"""
    ...
    async def _load_default_skills(self):
        ...
        # Load Python-format skills (legacy)
        default_skills = [
            './skills/core_tools.py',
            './skills/web_search.py',
            './skills/file_operations.py'
        ]
        for skill_path in default_skills:
            try:
                skill = await self.skill_loader.load_skill(Path(skill_path))
                if skill:
                    await self.skill_loader.activate_skill(skill.name)
                    self.active_skills[skill.name] = skill
                    self.logger.debug(f"已加载并激活技能：{skill.name}")
            except Exception as e:
                self.logger.warning(f"加载技能失败 {skill_path}：{e}")
