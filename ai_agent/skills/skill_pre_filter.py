...
class SkillPreFilter:
    ...
    def pre_filter(
        ...
        skill_feedback: 'SkillFeedback' = None,
    ) -> Tuple[List['SkillEntry'], Dict[str, float]]:
        """
        根据用户输入预筛选相关 skill
        ...
        Args:
            ...
            skill_feedback: SkillFeedback 实例，用于上下文 boost
        ...
        for name, kw_score in candidate_scores.items():
            ...
            # 4. 上下文 boost (0-15)：基于相似输入的历史调用效果
            if skill_feedback:
                context_boost = skill_feedback.get_context_boost(name, user_input)
                total += context_boost * 15
