    ...
    def __init__(self):
        super().__init__(
            ...
            description=(
                ...
                "When to use:\n"
                "- Reading a specific web page, article, or documentation\n"
                "- Getting the content of a URL found via web_search\n"
                "- Fetching API responses (JSON) or raw text from a URL\n\n"
                "Parameters:\n"
                "- url: the URL to fetch (http or https) (required)\n"
                "- max_chars: max characters to return (default: 5000, max: 50000)\n"
                "- extract_mode: 'text' for plain text (default), 'raw' for original content\n\n"
                "Returns: extracted text content with metadata (url, content_type, chars, truncated)\n\n"
                "Notes:\n"
                "- HTML pages are converted to plain text (scripts/styles removed)\n"
                "- JSON responses are returned as-is\n"
                "- Content exceeding max_chars is truncated with a notice\n"
                "- Internal/private URLs are blocked by SSRF protection"
            ),
            schema={
                "type": "object",
                "properties": {
                    "url": {
                        "type": "string",
                        "description": "URL to fetch (http or https). Must be a public URL."
                    },
                    "max_chars": {
                        "type": "integer",
                    ...
                    "extract_mode": {
                        "type": "string",
                        "description": "'text' to extract readable text from HTML (default), 'raw' to return original content",
                        "default": "text"
                    }
                },
                "required": ["url"]
