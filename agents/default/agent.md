...
### 前端效果红线
- **JS 渲染页面时禁止硬编码 CSS 参数**：`style="display:flex;gap:16px"` 等内联样式禁止出现在 JS 模板字符串中
- **只渲染标签**：JS 只负责生成 HTML 标签和 class 名，所有样式定义统一放在 CSS 文件中
- **动态值例外**：模板字面量计算的动态值（如条件 class、动态宽度百分比）可用内联 style，静态样式必须提取为 CSS 类
- **提取规则**：新增 CSS 类统一加 `js-` 前缀，避免与现有 class 冲突
