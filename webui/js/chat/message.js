...
export function chatAppendAgentMsg(text, meta) {
  ...
  const timeStr = new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'});
  const avatarUrl = chatCurrentAgent && chatCurrentAgent.avatar
    ? chatCurrentAgent.avatar : '/static/default_avatar.webp';
  row.innerHTML = `
  ...
  row.dataset.rawText = text;
  if (meta && (meta._raw || meta.tool_call_steps || meta.usage)) {
...
export function chatAddMessage(text, isAgent, meta, timestamp, scroll) {
  try {
    ...
    return row;
  } catch(e) { console.error('chatAddMessage error:', e); return null; }
}
      ...
      if (_chatStreamRow && _chatStreamBubble) {
        ...
        if (!_chatStreamRow) {
          ...
          if (container) {
            ...
            const avatarUrl = chatCurrentAgent && chatCurrentAgent.avatar ? chatCurrentAgent.avatar : '/static/default_avatar.webp';
