...
import {
  chatSessionId, chatCurrentAgent, chatPendingFiles,
...
export function openImageLightbox(src, name) {
  ...
  overlay.className = 'siper-image-lightbox';
  overlay.innerHTML = `<img src="${src}" alt="${chatEscapeHtml(name)}"><span class="siper-lightbox-close" onclick="this.parentElement.remove()">✕</span>`;
...
export async function loadChatModels() {
  try {
    ...
    updateChatHeader();
  } catch(e) { console.error('chatLoadModels error:', e); }
}
...
export function renderChatModelDropdown(models, showNoModels) {
  ...
  for (const m of models) {
    ...
    item.addEventListener('click', () => {
      ...
      if (window.toast) window.toast.success('模型切换为：' + (m.alias || m.name));
      updateCtxInfoDisplay();
      updateChatHeader();
    });
    menu.appendChild(item);
...
export function updateChatHeader() {
  ...
  if (!headerName || !chatSessionId || !chatCurrentAgent) return;
