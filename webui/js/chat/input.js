...
import { getWs, setWs } from '../core.js?v=1783611558619';
import {
  ...
  markSessionReady,
  setChatSessionId,
} from '../chat/state.js?v=1783611558619';
...
import { resetSendState } from '../chat/session.js?v=1783611558619';

// 全局待发送文件列表，存放 base64 数据、mime、名称及分类
window.chatPendingFiles = [];
let chatPendingFiles = window.chatPendingFiles;
import { chatAppendUserMsg, chatRenderMarkdown, chatEscapeHtml, updateCtxInfoDisplay } from './message.js?v=1783611558619';
import { renderMiddleList, refreshAgentsAndRender } from './sidebar.js?v=1783611558619';
...
import { chatThinkingShow, chatThinkingClear, chatThinkingAddTextRow, chatThinkingHide } from '../chat/thinking.js?v=1783611558619';
import { toast } from '../components/toast.js?v=1783611558619';
...
import { CAP_ICONS } from '../utils/capabilities.js?v=1783611558619';
...
export function renderChatModelDropdown(models, showNoModels) {
  ...
  for (const m of models) {
    ...
    item.addEventListener('click', () => {
      ...
      updateCtxInfoDisplay();
      updateChatHeader();
      // Sync model selection to backend session
      if (_chatSessionId) {
        fetch('/api/sessions/' + encodeURIComponent(_chatSessionId) + '/model', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
...
/** Fire-and-forget: touch session updated_at to current time */
function _touchSession(sid) {
  if (!sid) return;
  fetch('/api/sessions/' + encodeURIComponent(sid) + '/touch', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({})
  }).catch(function() {});
...
// Re-render model dropdown when agent's available_models changes
document.addEventListener('siper-models-changed', function(e) {
  var detail = e.detail || {};
  var currentAgent = window._chatCurrentAgent && window._chatCurrentAgent.name;
  if (detail.agent !== currentAgent) return;
  if (detail.models && detail.models.length) {
    renderChatModelDropdown(detail.models, false);
  }
});