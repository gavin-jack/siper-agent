...
const _CHAT_RENDERED_PAGES = new Set(['chat','skills','token','global-settings','logs','gateway']);
...
export function navigateToPage(page, skipHash) {
  ...
  // Pages rendered inside #page-chat three-column layout
  if (_CHAT_RENDERED_PAGES.has(page)) {
    // Ensure #page-chat is visible (hide all other .page first)
  ...
  // Legacy standalone pages (sessions, memory, agent-config, etc.)
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
  ...
  if (page === 'skills') { if (typeof refreshSkills === 'function') refreshSkills(); }
  if (page === 'memory') { populateMemoryAgentSelector(); refreshMemoryPage(); }
  if (page === 'agent-config') {
    // Clear session selection highlight in sidebar when entering agent config
    ...
    // Refresh sidebar to remove stale active-session highlight
  ...
  ws.onmessage = (e) => {
    try {
      ...
      if (d.type === 'stream_delta') {
        ...
        if (agent) {
          if (!agent.sessions.find(s => s.session_id === d.session_id)) {
          ...
          const newSession = agent.sessions.find(s => s.session_id === d.session_id);
          if (newSession && typeof window.selectChatSession === 'function') window.selectChatSession(newSession, agent);
        }
        if (currentPage === 'chat') {
          const chatEl = document.getElementById('chatMessages');
...
export function selectChatLang(lang) { return selectLang(lang); }
export function toggleChatLangDropdown() { return selectLang(); }
...
// ===== Upgrade =====
export function chatDoUpgrade() {
  if (typeof window.toast === 'function') toast.info('正在检查升级...');
  fetch('/api/upgrade', { method: 'POST' }).then(r => r.json()).then(d => {
    if (typeof window.toast === 'function') toast.success(d.message || '升级完成');
    if (d.restarted) setTimeout(() => location.reload(), 2000);
  }).catch(() => {
    if (typeof window.toast === 'function') toast.error('升级失败，请手动重启服务');
  });
}

export function startSidebarUpgradeTimer() {
  const statusEl = document.getElementById('sidebarUpgradeStatus');
  const btnEl = document.getElementById('sidebarUpgradeBtn');
  if (!statusEl) return;
  // 后端已改为后台线程缓存，前端只在页面加载时检查一次即可
  statusEl.textContent = '检查中...';
  fetch('/api/upgrade/check').then(r => r.json()).then(d => {
    if (d.has_updates) {
  ...
  }).catch(() => { statusEl.textContent = '检测失败（网络问题）'; });
}
