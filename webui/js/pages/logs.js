...
export function renderLogLevelFilters() {
  ...
  container.innerHTML = logState.availableLevels.map(lvl => {
    ...
    return `<span class="log-level-chip" data-level="${lvl}" style="cursor:pointer;padding:2px 8px;border-radius:4px;font-size:11px;font-weight:600;border:${border};background:${bg};color:${textColor};margin-right:4px;user-select:none;transition:all 0.15s">${lvl}</span>`;
  }).join('');
}
