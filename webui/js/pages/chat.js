...
if (typeof startSidebarUpgradeTimer === 'function') startSidebarUpgradeTimer();
...
export function chatSwitchPage(page, fromNavigate) {
  ...
  switch (page) {
    ...
    case 'gateway':   renderGatewayPageChat(content); break;
  }
}
...
export function renderChatPage(container) {
  ...
  // When no agent or session is selected, show default title instead of "对话"
  if (!showInput) {
    const headerName = document.getElementById('chatRightHeaderName');
  ...
  container.innerHTML = `
    ...
    <div class="siper-messages" id="chatMessages">
...
function renderSettingsPageChat(container) {
  ...
  fetch('/api/models/global').then(r => r.json()).then(md => { window._cachedGlobalModels = md; }).catch(() => {});
...
function renderLogsPageChat(container) {
  container.className = 'siper-content siper-full-content';
  container.innerHTML = `<div class="siper-page-toolbar" style="flex-wrap:wrap;gap:6px;"><button class="siper-btn" onclick="window.refreshLogs()">刷新</button><button class="siper-btn danger" onclick="window.clearLogs()">清空显示</button><input type="text" id="chatLogSearchInput" placeholder="搜索..." class="siper-input" style="width:140px;" oninput="window.applyLogLogsDebounced()" aria-label="日志搜索"><select id="chatLogLogLevel" class="siper-input" style="width:auto;" onchange="window.refreshLogs()" aria-label="日志级别"><option value="">全部级别</option><option value="DEBUG">DEBUG</option><option value="INFO">INFO</option><option value="WARN">WARN</option><option value="ERROR">ERROR</option></select><span id="chatLogStats" class="text-dim" style="font-size:12px;"></span></div><div id="chatLogsList" style="font-family:monospace;font-size:12px;line-height:1.8;"></div><div id="chatLogPagination"></div>`;
  if (typeof window.refreshLogs === 'function') window.refreshLogs();
}

function renderGatewayPageChat(container) {
  container.className = 'siper-content siper-full-content';
  container.innerHTML = `<div class="siper-page-toolbar"><button class="siper-btn" onclick="window.refreshGateway()">刷新</button></div><div id="chatGatewayList"></div>`;
  if (typeof window.refreshGateway === 'function') window.refreshGateway();
}
...
window.handleToolProgress = Stream.chatHandleToolProgress;
...
window.openImageLightbox = Input.openImageLightbox;
...
window.toggleModelDropdown = Input.toggleChatModelDropdown;
window.closeModelDropdown = Input.closeChatModelDropdown;
...
window.handleSearch = Sidebar.handleChatSearch;
...
window.chatLoadGlobalModels = () => {}; // mounted by app.js from Settings
...
window.copyMsg = copyChatMsg;
window.insertMsg = insertChatMsg;
