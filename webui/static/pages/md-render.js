...
function preprocessMarkdown(lines) {
  ...
  const processedLines = (() => {
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      const hrHeadingMatch = l.trim().match(/^(.*?)---+\s*(#{1,6}\s*.*)$/);
      if (hrHeadingMatch) {
        ...
        if (heading.match(/^#{1,6}\s*/)) {
          ...
          if (tableAfter) expanded.push(tableAfter);
          continue;
        }
      }
...
function processBlocks(text, lines, codeBlocks, frag, _maxIter) {
  ...
  // Normalize || to | before checking — || is just adjacent cell separators, not row breaks
  const _normalizePipes = (s) => s.replace(/\|\|/g, '|');
