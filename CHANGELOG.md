...
- **消息气泡时间格式化**：当日消息只显示 `HH:MM:SS`，昨天及以前显示 `YYYY-MM-DD HH:MM:SS`
...
- **会话折叠改为 agent 级触发**：去掉独立的 `_sessions` key，改为每次展开 agent 时检查会话数 >3 则折叠，折叠后再展开重新折叠

### Bug 修复 (fix)

- **stream.js finalizeStream 语法错误（补充修复）**：确保 if(parent) 块正确闭合，防止 ESM 加载失败
